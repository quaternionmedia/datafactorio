...
function handleFileChange(event: Event, cell: any) {
  ...
  if (fileInput.files && fileInput.files.length > 0) {
    ...
    cell.update({ inputFile: fileInput.files[0].name });
  }
}
...
export const FileInput = (cell) => {
  ...
  return m("div.menuitem", [
    ...
    // Custom input button
    m("button.browse", { onclick: triggerFileInput }, "Browse Files"),
    // Display file name
    //m("span.filename", cell.getState().inputFile || "No file chosen")
  ]);
};